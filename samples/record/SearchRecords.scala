...
import com.zoho.crm.api.record.RecordOperations.{SearchRecordsHeader, SearchRecordsParam}
...
object SearchRecords {
  ...
  @throws[Exception]
  private def searchRecords(moduleAPIName: String): Unit = {
    val recordOperations = new RecordOperations(moduleAPIName)
    val paramInstance = new ParameterMap()
//    paramInstance.add(new SearchRecordsParam().criteria, "((Company:starts_with:Zoho)or(Company:starts_with:Zoho))")
//    paramInstance.add(new SearchRecordsParam().email, "test@zoho.com")
//    paramInstance.add(new SearchRecordsParam().phone, "9876543210")
    paramInstance.add(new SearchRecordsParam().word, "Zoho")
//    paramInstance.add(new SearchRecordsParam().converted, "false")
//    paramInstance.add(new SearchRecordsParam().approved, "both")
//    paramInstance.add(new SearchRecordsParam().page, 1)
//    paramInstance.add(new SearchRecordsParam().perPage, 50)
    paramInstance.add(new SearchRecordsParam().fields, "Last_Name,Account_Name,First_Name,Lead_Source")
    
    val headerInstance = new HeaderMap()
//    headerInstance.add(new SearchRecordsHeader().XEXTERNAL, "Leads.External")

    val response: Option[APIResponse[ResponseHandler]] = recordOperations.searchRecords(Option(paramInstance), Option(headerInstance))
    ...
    if (response != null) {
      response match {
        ...
        case None => println("No response from the SearchRecords API.")
      }
    }
  }
