...
object UpdateModuleById {

  @throws[Exception]
  private def updateModuleById(): Unit = {
    val moduleId = 3477061000003905003L // Replace with actual module ID
    ...
    module.setAPIName(Option("CustomAPIName")) // Set custom API name if needed
    modules.addOne(module)
    
    val request = new BodyWrapper()
    request.setModules(modules)
    
    val response: Option[APIResponse[ActionHandler]] = modulesOperations.updateModule(moduleId, request)
