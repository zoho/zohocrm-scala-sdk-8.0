...
object UpdateNotifications {
  ...
  @throws[Exception]
  private def updateNotifications(): Unit = {
    val notificationsOperations = new NotificationsOperations()
    val notifications = new ArrayBuffer[Notification]()
    
    // Create notification with updated settings
    val notification = new Notification()
    notification.setChannelId(Option("1000000068001")) // Replace with actual channel ID
    notification.setNotifyUrl(Option("https://www.updated-example.com/notifications"))
    notification.setNotifyOnRelatedAction(Option(false)) // Updated setting
    
    val events = new ArrayBuffer[String]()
    events.addAll(Array("Leads.all", "Contacts.all", "Deals.create", "Accounts.edit"))
    notification.setEvents(events)
    notification.setToken(Option("updated_notification_token"))
    ...
    val response: Option[APIResponse[ActionHandler]] = notificationsOperations.updateNotifications(request)
