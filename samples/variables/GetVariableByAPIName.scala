...
object GetVariableByAPIName {

  @throws[Exception]
  private def getVariableByAPIName(apiName: String): Unit = {
    val variablesOperations = new VariablesOperations()
    
    // Create parameter map for optional filtering
    val paramInstance = new ParameterMap()
    paramInstance.add(new VariablesOperations.GetVariableByAPINameParam().group, "General")
    
    val response: Option[APIResponse[ResponseHandler]] = variablesOperations.getVariableByApiname(apiName, Option(paramInstance))
    ...
    if (response != null) {
      response match {
              ...
              case responseWrapper: ResponseWrapper =>
                ...
                if (variables != null && variables.nonEmpty) {
                  variables.foreach { variable =>
                    println(s"Variable Details for API Name: $apiName")
                    println("=" * 80)
        ...
        case None =>
          println("No response received from GetVariableByAPIName API.")
      }
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      val apiName: String = "UpdatedVariable"
      getVariableByAPIName(apiName)
    } catch {
