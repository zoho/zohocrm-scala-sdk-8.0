...
object GetVariables {
  ...
  @throws[Exception]
  private def getVariables(): Unit = {
    val variablesOperations = new VariablesOperations()
    
    // Create parameter map for optional filtering
    val paramInstance = new ParameterMap()
    paramInstance.add(new VariablesOperations.GetVariablesParam().group, "General")
    
    val response: Option[APIResponse[ResponseHandler]] = variablesOperations.getVariables(Option(paramInstance))
    ...
    if (response != null) {
      response match {
        ...
        case None =>
          println("No response received from GetVariables API.")
      }
    }
  }
