...
class ActionWrapper extends Model with ActionHandler	{
	private var globalPicklists:ArrayBuffer[ActionResponse] = _
	private var keyModified:HashMap[String, Int] = HashMap()

	def getGlobalPicklists() :ArrayBuffer[ActionResponse]	={
		return  this.globalPicklists
	}

	def setGlobalPicklists( globalPicklists: ArrayBuffer[ActionResponse]) 	={
