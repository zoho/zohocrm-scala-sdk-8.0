...
import com.zoho.crm.api.currencies.{ActionHandler, BaseCurrencyActionWrapper, APIException, BaseCurrency, BaseCurrencyWrapper, CurrenciesOperations, SuccessResponse}
import com.zoho.crm.api.dc.DataCenter.Environment
import com.zoho.crm.api.dc.INDataCenter
import com.zoho.crm.api.util.APIResponse

object UpdateBaseCurrency {

  @throws[Exception]
  private def updateBaseCurrency(): Unit = {
    val currenciesOperations = new CurrenciesOperations()
    val request = new BaseCurrencyWrapper()
    val baseCurrency = new BaseCurrency()
    baseCurrency.setId(Option(347706100L))
    baseCurrency.setPrefixSymbol(Option(false))
    baseCurrency.setIsoCode(Option("USD"))
    baseCurrency.setExchangeRate(Option("1.0"))
    
    request.setBaseCurrency(Option(baseCurrency))
    
    val response: Option[APIResponse[ActionHandler]] = currenciesOperations.updateBaseCurrency(request)
