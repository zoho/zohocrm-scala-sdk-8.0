...
object AddCurrencies {

  @throws[Exception]
  private def addCurrencies(): Unit = {
    val currenciesOperations = new CurrenciesOperations()
    val request = new BodyWrapper()
    val currencies = new ArrayBuffer[Currency]()
    val currency1 = new Currency()
    
    currency1.setName(Option("Algerian Dinar"))
    currency1.setIsoCode(Option("DZD"))
    currency1.setSymbol(Option("DZD"))
    currency1.setExchangeRate(Option("1.0"))
    
    currencies.addOne(currency1)
    request.setCurrencies(currencies)
    
    val response: Option[APIResponse[ActionHandler]] = currenciesOperations.addCurrencies(request)
                      ...
                      case successResponse: SuccessResponse =>
                        ...
                        successResponse.getDetails().foreach { details =>
                          details.foreach { case (key, value) =>
                            println(s"$key: $value")
                            if (key == "id") {
                              println(s"Currency created successfully. Currency ID: $value")
                            }
                          }
                        }
