...
object UpdateRoles {
  ...
  private def updateRoles(): Unit = {
    ...
    contactRole1.setId(Option(1055806000010280001L)) // Replace with actual contact role ID
    contactRole1.setName(Option("Updated Senior Manager"))
    ...
    contactRole2.setId(Option(1055806000000661002L)) // Replace with actual contact role ID
    contactRole2.setName(Option("Updated Assistant Manager"))
    contactRole2.setSequenceNumber(Option(2))
    contactRoles.addOne(contactRole2)

    request.setContactRoles(contactRoles)

    val response: Option[APIResponse[ActionHandler]] = contactRolesOperations.updateRoles(request)
