...
import com.zoho.crm.api.relatedrecords.RelatedRecordsOperations.UpdateRelatedRecordHeader
...
object UpdateRelatedRecord {

  @throws[Exception]
  private def updateRelatedRecord(moduleAPIName: String, recordId: Long, relatedListAPIName: String, relatedRecordId: Long): Unit = {
    ...
    val response: Option[APIResponse[ActionHandler]] = relatedRecordsOperations.updateRelatedRecord(
      relatedRecordId, 
      recordId, 
      bodyWrapper, 
    ...
    if (response != null) {
      response match {
        ...
        case None => 
          println("No response received from UpdateRelatedRecord API.")
      }
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      val relatedRecordId = 1055806000006611017L  // Contact record ID to update
      
      updateRelatedRecord(moduleAPIName, recordId, relatedListAPIName, relatedRecordId)
    } catch {
