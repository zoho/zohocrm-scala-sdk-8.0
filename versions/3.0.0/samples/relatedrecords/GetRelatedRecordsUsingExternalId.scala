...
import com.zoho.crm.api.relatedrecords.RelatedRecordsOperations.{GetRelatedRecordsUsingExternalIDHeader, GetRelatedRecordsUsingExternalIDParam}
...
object GetRelatedRecordsUsingExternalId {

  @throws[Exception]
  private def getRelatedRecordsUsingExternalId(moduleAPIName: String, externalValue: String, relatedListAPIName: String): Unit = {
    val relatedRecordsOperations = new RelatedRecordsOperations(relatedListAPIName, moduleAPIName)
    
    val paramInstance = new ParameterMap()
    paramInstance.add(new GetRelatedRecordsUsingExternalIDParam().page, 1)
    paramInstance.add(new GetRelatedRecordsUsingExternalIDParam().perPage, 50)
    paramInstance.add(new GetRelatedRecordsUsingExternalIDParam().fields, "Last_Name,First_Name,Email,Phone")
    
    val headerInstance = new HeaderMap()
    headerInstance.add(new GetRelatedRecordsUsingExternalIDHeader().XEXTERNAL, "Leads.External")

    val response: Option[APIResponse[ResponseHandler]] = relatedRecordsOperations.getRelatedRecordsUsingExternalId(
      externalValue, 
    ...
    if (response != null) {
      response match {
        ...
        case None => 
          println("No response received from GetRelatedRecordsUsingExternalId API.")
      }
    }
  }
