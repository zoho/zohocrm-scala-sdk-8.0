...
import com.zoho.crm.api.relatedrecords.RelatedRecordsOperations.{GetRelatedRecordUsingExternalIDHeader, GetRelatedRecordUsingExternalIDParam}
...
object GetRelatedRecordUsingExternalId {

  @throws[Exception]
  private def getRelatedRecordUsingExternalId(moduleAPIName: String, externalValue: String, relatedListAPIName: String, externalFieldValue: String): Unit = {
    val relatedRecordsOperations = new RelatedRecordsOperations(relatedListAPIName, moduleAPIName)
    
    val paramInstance = new ParameterMap()
    paramInstance.add(new GetRelatedRecordUsingExternalIDParam().fields, "Last_Name,First_Name,Email,Phone,Mailing_Street,Department,Title")
    
    val headerInstance = new HeaderMap()
    headerInstance.add(new GetRelatedRecordUsingExternalIDHeader().XEXTERNAL, "Leads.External,Products.Products_External")

    val response: Option[APIResponse[ResponseHandler]] = relatedRecordsOperations.getRelatedRecordUsingExternalId(
      externalFieldValue, 
      externalValue, 
    ...
    if (response != null) {
      response match {
        ...
        case None => 
          println("No response received from GetRelatedRecordUsingExternalId API.")
      }
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      val externalFieldValue = "34770614994115"  // External ID of the contact record
      
      getRelatedRecordUsingExternalId(moduleAPIName, externalValue, relatedListAPIName, externalFieldValue)
    } catch {
