...
object GetRelatedRecords {

  @throws[Exception]
  private def getRelatedRecords(moduleAPIName: String, recordId: Long, relatedListAPIName: String): Unit = {
    ...
    paramInstance.add(new GetRelatedRecordsParam().perPage, 50)
    paramInstance.add(new GetRelatedRecordsParam().fields, "Last_Name,First_Name,Email,Phone")
//    paramInstance.add(new GetRelatedRecordsParam().ids, "1055806000001393066")
    
    val headerInstance = new HeaderMap()
    val response: Option[APIResponse[ResponseHandler]] = relatedRecordsOperations.getRelatedRecords(
      recordId, 
    ...
    if (response != null) {
      response match {
        ...
        case None => 
          println("No response received from GetRelatedRecords API.")
      }
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      val recordId = 1055806000028754070L
