...
object GetVariableGroupByAPIName {

  @throws[Exception]
  private def getVariableGroupByAPIName(apiName: String): Unit = {
    val variableGroupsOperations = new VariableGroupsOperations()
    
    val response: Option[APIResponse[ResponseHandler]] = variableGroupsOperations.getVariableGroupByAPIName(apiName)
    ...
    if (response != null) {
      response match {
                ...
                responseHandler match {
                  case responseWrapper: ResponseWrapper =>
                    ...
                    if (variableGroups != null && variableGroups.nonEmpty) {
                      ...
                      println(s"No variable group found with API name: $apiName")
                    }
        ...
        case None =>
          println("No response received from GetVariableGroupByAPIName API.")
      }
    }
  }
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      getVariableGroupByAPIName("variable_group_api_name") // Replace with actual API name
    } catch {
