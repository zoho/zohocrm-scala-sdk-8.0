package com.zoho.crm.api.unsubscribelinks

import com.zoho.crm.api.util.Model
import scala.collection.mutable.HashMap
import scala.collection.mutable.ArrayBuffer

class AssociationDetails extends Model	{
	private var id:Option[Long] = None
	private var associatedPlaces:ArrayBuffer[AssociatedPlaces] = _
	private var keyModified:HashMap[String, Int] = HashMap()

	def getId() :Option[Long]	={
		return  this.id
	}

	def setId( id: Option[Long]) 	={
		 this.id = id
		 this.keyModified("id") = 1
	}

	def getAssociatedPlaces() :ArrayBuffer[AssociatedPlaces]	={
		return  this.associatedPlaces
	}

	def setAssociatedPlaces( associatedPlaces: ArrayBuffer[AssociatedPlaces]) 	={
		 this.associatedPlaces = associatedPlaces
		 this.keyModified("associated_places") = 1
	}

	def isKeyModified( key: String) :Any	={
		if((( this.keyModified.contains(key))))
		{
			return  this.keyModified(key)
		}
		return None
	}

	def setKeyModified( key: String,  modification: Int) 	={
		 this.keyModified(key) = modification
	}}