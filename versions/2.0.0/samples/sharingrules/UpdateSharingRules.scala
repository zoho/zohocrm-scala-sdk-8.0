...
object UpdateSharingRules {

  def updateSharingRules(moduleAPIName: String): Unit = {
    ...
    sharingRule.setId(Option(3477051001L))

    sharingRules.addOne(sharingRule)
    request.setSharingRules(sharingRules)

    val response = sharingRulesOperations.updateSharingRules(request)
